...
const ChapterConfiguration: React.FC<ChapterConfigurationProps> = ({
  previousContent,
}) => {
  ...
  const [currentChapterNo, setCurrentChapterNo] = useState(() => {
    return location.state?.initialChapter
      ? Number(location.state.initialChapter)
      : 1;
  });
  ...
  const [activeEndContentType, setActiveEndContentType] = useState<
  ...
  const handleRegenerateStreamedContent = (
    content: string,
    isNewChunk = true
  ) => {
    if (isNewChunk) {
  ...
  const formatImageUrl = (url: string) => {
    ...
    try {
      ...
      return baseUrl
        ? formattedUrl.replace("http://localhost:3000", baseUrl)
        : formattedUrl;
    } catch (error) {
  ...
  const handleRegenerateParagraph = async (
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (error instanceof Error) {
            ...
            addToast("streaming issue", ToastType.ERROR);
          }
      ...
      if (error instanceof Error) {
        ...
        addToast("error on chapter regenerate .", ToastType.ERROR);
      }
      setIsGenerating(false);
    }
  };
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const bookId =
        typeof bookData.id === "string"
          ? parseInt(bookData.id, 10)
          : bookData.id;
    ...
    if (selectedContent) {
      try {
        ...
        const contentToInsert =
          editMode === "ai" ? regeneratedContent : humanEditContent;
        ...
        if (error instanceof Error) {
          ...
          addToast("error on update changings .", ToastType.ERROR);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    if (
      ...
      Array.isArray(bookData.bookChapter)
    ) {
      // Find chapters that already exist
      const existingChapters = bookData.bookChapter.map(
        (chapter: { chapterNo: any }) => chapter.chapterNo
      );
      ...
      if (location.state?.initialChapter) {
        ...
        const nextChapterToGenerate = findNextChapterToGenerate(
          existingChapters,
          bookData.numberOfChapters
        );
        ...
        if (
          existingChapters.includes(initialChapter) &&
          nextChapterToGenerate !== initialChapter
        ) {
          setCurrentChapterNo(nextChapterToGenerate);
          addToast(
            `Redirecting to chapter ${nextChapterToGenerate} which needs to be generated`,
            ToastType.INFO
          );
        }
      }
  ...
  const findNextChapterToGenerate = (
  ...
  useEffect(() => {
    if (
      bookData &&
      bookData.type === "incomplete" &&
      bookData.numberOfChapters <= bookData.bookChapter?.length
    ) {
      // Check if any of the end content sections are missing
      const needsEndContent =
        !bookData.glossary || !bookData.index || !bookData.references;
        ...
        <>
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">
              Generating chapter {currentChapterNo} of{" "}
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor="ai-edit" className="cursor-pointer">
                        AI
                      </Label>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor="human-edit" className="cursor-pointer">
                        Human
                      </Label>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleChange("minLength", e.target.value)
                        }
                        className="mt-1"
                        placeholder="500"
                      />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleChange("noOfImages", e.target.value)
                      }
                      className="mt-1"
                      placeholder="0"
                    />
            ...
            <Card className="lg:col-span-2 p-6 h-full">
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mt-2 text-center">
                  Chapter {currentChapterNo} of {getChapterCount()}:{" "}
                  {getCurrentChapterTitle()}
                </p>
              </div>
